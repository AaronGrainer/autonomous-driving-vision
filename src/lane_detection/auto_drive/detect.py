...
from src.lane_detection.auto_drive.utils import prob_to_lines
...
class LaneDetector:
    def __init__(self):
        ...
        with open("src/lane_detection/auto_drive/configs.yaml") as f:
    ...
    def _predict(self, img):
        ...
        transforms = Compose(
            [Resize(size=self.input_sizes), ToTensor(), Normalize(mean=self.mean, std=self.std)]
        )
        ...
        with autocast(self.mixed_precision):
            ...
            prob_map = F.interpolate(
                output["out"], size=self.input_sizes, mode="bilinear", align_corners=True
            ).softmax(dim=1)
            ...
            if self.max_lane != 0:  # Lane max number prior for testing
                ...
                while (existence.sum(dim=1) > self.max_lane).sum() > 0:
                    indices = (existence.sum(dim=1, keepdim=True) > self.max_lane).expand_as(
                        existence
        ...
        for j in range(existence.shape[0]):
            lane_coordinates = prob_to_lines(
                prob_map[j],
                existence[j],
