...
from src.lane_detection.data.constant import culane_row_anchor
from src.lane_detection.data.dataset import LaneTestDataset
from torchvision import transforms

from src.lane_detection.parsing_net.config import global_config as gc
from src.lane_detection.parsing_net.model.model import ParsingNet
        ...
        for i in range(out_j.shape[1]):
            if np.sum(out_j[:, i] != 0) > 2:
                for k in range(out_j.shape[0]):
                    if out_j[k, i] > 0:
                        ppp = (
                            int(out_j[k, i] * self.col_sample_w * self.img_w / 800) - 1,
                            int(
                                self.img_h
                                * (culane_row_anchor[self.cls_num_per_lane - 1 - k] / 288)
                            )
                            - 1,
                        )
...
class LaneDetectionCV:
    ...
    def _weighted_img(self, img, initial_img, α=0.95, β=1.0, γ=0.0):
    ...
    def detect(self, img):
        ...
        vertices = np.array(
            [[(0, img.shape[0]), (img.shape[1], img.shape[0]), (400, 260), (600, 260)]]
        )
