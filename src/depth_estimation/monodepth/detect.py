...
import src.depth_estimation.monodepth.networks as networks
from src.depth_estimation.monodepth.layers import disp_to_depth
from src.depth_estimation.monodepth.utils import download_model_if_doesnt_exist
...
class MonoDepthEstimator:
    def __init__(
        self, dir_path="depth_estimation/monodepth/models", model_name="mono+stereo_640x192"
    ):
    ...
    def _get_bbox_depth(self, metric_depth, preds, class_only):
        ...
        for _, pred in preds.iterrows():
            if pred["name"] in class_only:
                box_depth = metric_depth[0][0][
                    int(pred["ymin"]) : int(pred["ymax"]), int(pred["xmin"]) : int(pred["xmax"])
                ]
