...
def check_anchor_order(m):
    """Check anchor order against stride order for YOLOv5 Detect()
    module m, and correct if necessary
    """
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    if bpr > 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            ...
            LOGGER.info(
                f"{PREFIX}New anchors saved to model. Update model *.yaml "
                "to use these anchors in the future."
            )
        else:
            LOGGER.info(
                f"{PREFIX}Original anchors better than new anchors. "
...
def kmean_anchors(
    ...
    def print_results(k, verbose=True):
        ...
        s = (
            f"{PREFIX}thr={thr:.2f}: {bpr:.4f} best possible recall, "
            f"{aat:.2f} anchors past thr\n{PREFIX}n={n}, img_size={img_size}, "
    ...
    if i:
        LOGGER.info(
            f"{PREFIX}WARNING: Extremely small objects found. "
    ...
    progress_bar = tqdm(range(gen), desc=f"{PREFIX}Evolving anchors with Genetic Algorithm:")
