...
from src.vision.v5.utils.augmentations import letterbox
from src.vision.v5.utils.datasets import exif_transpose
from src.vision.v5.utils.general import (
    colorstr,
    increment_path,
    is_ascii,
    make_divisible,
...
from src.vision.v5.utils.plots import Annotator, colors, save_one_box
from src.vision.v5.utils.torch_utils import copy_attr, time_sync
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, version="r4.0"):
        ...
        if version == "r4.0":
            ...
            self.act = (
                nn.Hardswish() if act else (act if isinstance(act, nn.Module) else nn.Identity())
            )
        else:
