...
from src.vision.data._helper import create_small_table
from src.vision.data.distributed import all_gather
...
class COCOEvaluator(Metric):
    ...
    def update(self, preds, targets):
        records = {
            target["image_id"].item(): prediction for target, prediction in zip(targets, preds)
        }
    ...
    def derive_coco_results(self, class_names: Optional[List[str]] = None):
        ...
        results = {
            metric: float(
                self.coco_eval.stats[idx] * 100 if self.coco_eval.stats[idx] >= 0 else "nan"
            )
