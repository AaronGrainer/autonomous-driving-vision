...
def get_affine_matrix(target_size, degrees=10, translate=0.1, scales=0.1, shear=10):
    ...
    scale = get_aug_params(scale, center=1.0)
...
def augment_hsv(img, hgain=5, sgain=5, vgain=30):
...
def _mirror(image, boxes, prob=0.5):
    ...
    if random.random() < prob:
        image = image[:, ::-1]
        boxes[:, 0::2] = width - boxes[0, 2::-2]
    return image, boxes


class TrainTransform:
    def __init__(self, max_labels: int = 50, flip_prob: int = 0.5, hsv_prob: int = 1.0):
    ...
    def __call__(self, image, targets, input_dim):
        ...
        padded_labels[range(len(targets_t))[:, self.max_labels]] = targets[: self.max_labels]
