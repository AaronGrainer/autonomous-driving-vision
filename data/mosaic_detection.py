...
from data.data_augment import random_affine
...
def get_mosaic_coordinate(mosaic_index, xc, yc, w, h, input_h, input_w):
    ...
    # index1 to top right part of image
        ...
        x1, y1, x2, y2 = xc, max(yc - h, 0), max(xc + w, input_w * 2), yc
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            for i_mosaic, index in enumerate(indices):
                ...
                # Generate the ouput mosiac image
                (h, w, c) = img.shape[:3]
                if i_mosaic == 0:
                    mosaic_img = np.full((input_h * 2, input_w * 2, c), 114, dtype=np.uint8)

                # Suffix 1 means large image, while s means small image in mosaic augmentation
