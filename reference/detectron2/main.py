...
def run_instance_segmentation_model():
    ...
    cv2.imshow('Detectron2 Instance Prediction', out.get_image()[:, :, ::-1])
    cv2.waitKey()
...
def run_panoptic_segmentation_model():
    im = cv2.imread("./input.jpg")
    # cv2.imshow('Input Image', im)
    # cv2.waitKey()
...
def evaluate(cfg, balloon_metadata, trainer):
